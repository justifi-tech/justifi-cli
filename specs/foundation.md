...
| Command layer | Only resource-specific validation/orchestration (mutual exclusions, required-flag errors, enum validation, file upload/download) | Direct tests of the validation functions; a consumer-defined gomock only where a canned client response is needed |
...
**Mocking rules** (see [http-client.md](http-client.md), [resource-command-template.md](resource-command-template.md)):
- Mocks are generated by `gomock`; never handwritten.
- Interfaces are defined by the **consumer**, and the mock is generated next to that interface.
- No test asserts a mock against the real implementation — generated mocks cannot drift, so such a test verifies nothing.
